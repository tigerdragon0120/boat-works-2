...
export default function PlayerPhoto({ src, registrationNumber, alt, size = "md" }) {
  ...
  if (!currentSrc) {
    return (
      <div className={cn("rounded-full bg-slate-100 border border-slate-600 flex items-center justify-center shrink-0", sz)}>
  ...
  return (
    <img
      ...
      className={cn("rounded-full object-cover border border-slate-600 shrink-0 bg-slate-100", sz)}
    />
  );
}