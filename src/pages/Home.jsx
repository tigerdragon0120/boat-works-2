  ...
  return (
    <div className="text-slate-100 space-y-5">
      <div className="flex items-center justify-between gap-3">
        <div><h1 className="text-2xl font-black tracking-tight">本日のレース場</h1><p className="text-xs text-slate-500 mt-1">レース場を選んで、1R〜12Rの予想へ進みます</p></div>
        <button onClick={load} className="h-9 px-3 rounded-lg border border-slate-700 bg-slate-900 text-slate-300 hover:bg-slate-800 flex items-center gap-2 text-xs font-semibold"><RefreshCw className={cn("w-4 h-4", loading && "animate-spin")} />更新</button>
      </div>

      <section className="rounded-2xl border border-slate-800 bg-[#0b1118] p-4 sm:p-5">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5">
          {VENUES.map(([code, name]) => {
            ...
            return active ? <Link key={code} to={`/venue/${code}`} className="rounded-xl border border-blue-500/30 bg-gradient-to-b from-blue-600/15 to-slate-900 p-3 hover:border-blue-400 hover:bg-blue-500/15 transition shadow-sm">{content}</Link> : <div key={code} className="rounded-xl border border-slate-800 bg-slate-900/50 p-3 opacity-70">{content}</div>;
          })}
        </div>
      </section>
