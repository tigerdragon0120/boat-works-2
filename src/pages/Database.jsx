...
import { Database as DbIcon, Users, MapPin, Gauge, Search, RefreshCw, Activity } from 'lucide-react';
import { cn } from '@/lib/utils';
import DatabaseStatusPanel from '@/components/DatabaseStatusPanel';

const tabs=[['profiles','選手詳細',Activity],['racers','選手DB',Users],['venues','レース場DB',MapPin],['motors','モーターDB',Gauge],['races','レースDB',DbIcon]];
export default function Database(){
  const [tab,setTab]=useState('profiles'); const [q,setQ]=useState(''); const [busy,setBusy]=useState(false); const [data,setData]=useState({profiles:[],racers:[],venues:[],motors:[],races:[]}); const [dbStatus,setDbStatus]=useState(null);
  ...
  const load=async()=>{ const [profiles,racers,venues,motors,races]=await Promise.all([
    base44.entities.RacerPerformanceProfile.list('-updated_at',500), base44.entities.RacerDatabase.list('-latest_race_date',500), base44.entities.VenueDatabase.list('venue_code',100), base44.entities.MotorDatabase.list('-top2_rate',500), base44.entities.RaceDatabase.list('-race_date',500)
  ]); setData({profiles,racers,venues,motors,races}); };
  ...
  return <div className="space-y-3">
    ...
    {tab==='profiles'&&<Profiles rows={rows}/>} {tab==='racers'&&<Racers rows={rows}/>} {tab==='venues'&&<Venues rows={rows}/>} {tab==='motors'&&<Motors rows={rows}/>} {tab==='races'&&<Races rows={rows}/>} 
  </div>
}
...
function Profiles({rows}){return <div className="space-y-2">{rows.map(r=><ProfileCard key={r.registration_number} p={r}/>)}</div>}
function ProfileCard({p}){
  const s6=p.stats_6m||{}, s1=p.stats_1y||{}, s3=p.stats_3y||{}, sAll=p.stats_all||{};
  const wm=p.winning_methods||{}, rf=p.recent_form||{};
  const cs=p.course_stats||{};
  return <Card><div className="p-3">
    <div className="flex items-center gap-3 mb-3">
      <PlayerPhoto registrationNumber={p.registration_number} alt={p.racer_name} size="sm"/>
      <div className="min-w-0 flex-1">
        <div className="font-bold text-white truncate">{p.racer_name||'—'} <span className="text-[10px] text-slate-500">#{p.registration_number}</span></div>
        <div className="text-[10px] text-slate-500">{p.grade_class||'—'}・標本 {p.total_samples||0}件・信頼度 {p.data_confidence!=null?Math.round(p.data_confidence):'—'}</div>
      </div>
    </div>
    <div className="grid grid-cols-4 gap-2 mb-2">
      <PeriodStat label="6ヶ月" s={s6}/>
      <PeriodStat label="1年" s={s1}/>
      <PeriodStat label="3年" s={s3}/>
      <PeriodStat label="全期間" s={sAll}/>
    </div>
    <div className="grid grid-cols-6 gap-1 mb-2">
      {[1,2,3,4,5,6].map(n=>{const c=cs[String(n)]||{};return <div key={n} className="text-center rounded bg-[#161a22] py-1"><div className="text-[9px] text-slate-500">{n}コース</div><div className="text-[10px] font-bold text-slate-300">{c.sample_size?`${p2(c.win_rate)}%`:'—'}</div><div className="text-[8px] text-slate-500">{c.sample_size||0}件</div></div>})}
    </div>
    {wm.total_wins>=2&&<div className="flex flex-wrap gap-1.5 text-[10px]">
      <Kimarite label="逃げ" v={wm.escape_rate}/>
      <Kimarite label="差し" v={wm.sashi_rate}/>
      <Kimarite label="まくり" v={wm.makuri_rate}/>
      <Kimarite label="まくり差し" v={wm.makuri_sashi_rate}/>
      <Kimarite label="抜き" v={wm.nuki_rate}/>
    </div>}
    {rf.recent_5_avg_finish!=null&&<div className="text-[10px] text-slate-500 mt-2">最近5走平均着順 {n(rf.recent_5_avg_finish)}・最近勝率 {p2(rf.recent_win_rate)}・勢い {rf.momentum_score!=null?n(rf.momentum_score,2):'—'}</div>}
  </div></Card>
}
function PeriodStat({label,s}){return <div className="rounded bg-[#161a22] p-2"><div className="text-[9px] text-slate-500 mb-1">{label} ({s.sample_size||0}件)</div><div className="grid grid-cols-3 gap-1 text-center"><div><div className="text-[8px] text-slate-500">1着</div><div className="text-[11px] font-bold text-amber-300">{p2(s.win_rate)}</div></div><div><div className="text-[8px] text-slate-500">2連</div><div className="text-[11px] font-bold text-slate-200">{p2(s.top2_rate)}</div></div><div><div className="text-[8px] text-slate-500">3連</div><div className="text-[11px] font-bold text-slate-200">{p2(s.top3_rate)}</div></div></div></div>}
function Kimarite({label,v}){return <span className="px-1.5 py-0.5 rounded bg-[#161a22] border border-[#2d3748] text-slate-400">{label} {v!=null?`${Number(v).toFixed(0)}%`:'—'}</span>}
function p2(v){return v==null?'—':`${Number(v).toFixed(1)}%`}
