// ============================================================
// Prediction Service V6.1 (サーバー側)
// V6.1 PROFIT Candidate予想をDBへ保存・検証。V2〜V5とは完全独立。
//
// 保存:
...
// 既存V6予想取得(重複作成防止)
async function getOrCreateV61Prediction(client, raceId, raceKey, stage) {
  const list = await client.asServiceRole.entities.PredictionV6.filter(
    { race_id: raceId, stage, prediction_version: V61_VERSION },
    "-computed_at", 1
  ).catch(() => []);
  if (list && list[0]) return { id: list[0].id, existing: list[0] };
  const created = await client.asServiceRole.entities.PredictionV6.create({
    race_id: raceId, race_key: raceKey, stage, prediction_version: V61_VERSION, status: "PENDING",
  }).catch(() => null);
...
// V6予想を実行して保存
// ============================================================
export async function runAndSavePredictionV61(client, race, entries, settings, stage, oddsMap = {}, profileByReg = null, rollingByReg = null) {
  try {
    ...
    // V6予想実行
    const result = runPredictionV61(entriesWithProfiles, race, settings, {
      stage, oddsMap: effectiveOddsMap,
    });

    // V6予想レコード保存
    const { id: predictionId, existing } = await getOrCreateV61Prediction(client, race.id, race.race_key, stage);
    if (!predictionId) {
      console.error("[V6.1] Failed to create prediction record");
    ...
    const record = {
      race_id: race.id, race_key: race.race_key, stage, prediction_version: V61_VERSION,
      ...
      v61_weights: result.v61_weights,
    ...
    await sr.PredictionV6.update(predictionId, record).catch(e => {
      console.error("[V6.1] Failed to save prediction:", e.message);
    });
    ...
    if (stage === "FINAL" && result.final_judgment === "BUY") {
      try {
        const existingLearning = await sr.PredictionLearningSample.filter(
          { race_id: race.id, stage: "FINAL", prediction_version: V61_VERSION }, "-created_at", 1
        ).catch(() => []);
        if (!existingLearning?.length) {
          await sr.PredictionLearningSample.create({
            race_id: race.id,
            stage: "FINAL",
            prediction_version: V61_VERSION,
            snapshot: {
        ...
        console.warn(`[V6.1] learning snapshot save skipped race=${race.id}:`, e.message);
      }
    }

    return { predictionId, result, skipped: false };
  } catch (e) {
    console.error(`[V6.1] runAndSavePredictionV61 error race=${race?.id} stage=${stage}:`, e.message);
    return { skipped: true, reason: "V61_ERROR", error: e.message };
  }
}
...
// factor_snapshotを予想時点から検証レコードへコピー
// ============================================================
export async function verifyV61Prediction(client, race, resultData) {
  try {
    const sr = client.asServiceRole.entities;
    ...
    let v6Final = await sr.PredictionV6.filter(
      { race_id: race.id, stage: "FINAL", prediction_version: "v6.1" }, "-computed_at", 1
    ).catch(() => []);
    if (!v6Final?.length && race.race_key) {
      v6Final = await sr.PredictionV6.filter(
        { race_key: race.race_key, stage: "FINAL", prediction_version: "v6.1" }, "-computed_at", 1
      ).catch(() => []);
    }
    let v6Pre = await sr.PredictionV6.filter(
      { race_id: race.id, stage: "PRE", prediction_version: "v6.1" }, "-computed_at", 1
    ).catch(() => []);
    if (!v6Pre?.length && race.race_key) {
      v6Pre = await sr.PredictionV6.filter(
        { race_key: race.race_key, stage: "PRE", prediction_version: "v6.1" }, "-computed_at", 1
      ).catch(() => []);
    }
    ...
    const verifDoc = {
      ...
      v61_pre_prediction: v6PrePred?.top_trifecta || "",
      v61_final_prediction: v6FinalPred?.top_trifecta || "",
      v61_pre_hit: v6PreHit, v61_final_hit: v6FinalHit, v61_recommended_hit: v6RecommendedHit,
      v61_final_judgment: v6FinalPred?.final_judgment || null,
      v61_ticket_count: v6FinalPred?.ticket_count || null,
      v61_selected_trifectas: v6FinalPred?.selected_trifectas || [],
      v61_payout: v6Payout, v61_investment: v6Investment, v61_recovery_rate: v6Recovery,
      v61_first_boat: v6FinalPred?.honmei_boat || v6PrePred?.honmei_boat || null,
      v61_escape_probability: v6FinalPred?.escape_probability ?? v6PrePred?.escape_probability ?? null,
      v61_winning_scenario: v6FinalPred?.winning_scenario || v6PrePred?.winning_scenario || null,
    ...
    // upsert
    const existing = await sr.PredictionV61Verification.filter({ race_id: race.id }, "-verified_at", 1).catch(() => []);
    let saved;
    if (existing?.[0]) saved = await sr.PredictionV61Verification.update(existing[0].id, verifDoc);
    else saved = await sr.PredictionV61Verification.create(verifDoc);
    ...
    const learningSamples = await sr.PredictionLearningSample.filter(
      { race_id: race.id, stage: "FINAL", prediction_version: V61_VERSION }, "-created_at", 10
    ).catch(() => []);
    ...
    console.error(`[V6.1] verifyV61Prediction error race=${race?.id}:`, e.message);
    return null;
  }
}