// ============================================================
// Prediction Engine V6.1 PROFIT Candidate
// 回収率110%+を最重要目標とする利益型予想エンジン
// V2〜V6とは完全独立。V6本体を変更しない。
//
// 核心思想:
...
const V61_WEIGHTS = { past: 0.25, recent: 0.35, today: 0.40 };
...
function computeV6BoatScores(entries, race, stage) {
  ...
  const boats = activeEntries.map(entry => {
    ...
    if (today.score != null) {
      pre_score = clamp(
        past.score * V61_WEIGHTS.past + recent.score * V61_WEIGHTS.recent + today.score * V61_WEIGHTS.today,
        0, 100
      );
    } else {
      const totalW = V61_WEIGHTS.past + V61_WEIGHTS.recent;
      pre_score = clamp(
        (past.score * V61_WEIGHTS.past + recent.score * V61_WEIGHTS.recent) / totalW,
        0, 100
      );
    }
  ...
  if (isFinal) {
    for (const b of boats) {
      const finalScore = clamp(
        b.past_score * V61_WEIGHTS.past + b.recent_score * V61_WEIGHTS.recent + b.today_score * V61_WEIGHTS.today,
        0, 100
      );
...
// セット期待値計算
// ============================================================
function computeV61SetMetrics(selected, trifectas, oddsMap) {
  ...
  // V6.1: 各買い目の確率×オッズを個別に合算する。
  // 旧V6の「平均オッズ×セット確率」は確率とオッズの逆相関を無視し、
  // 高配当側の期待値を過大評価するため使用しない。
  const expectedRecovery = selectedData.length > 0 && oddsValues.length === selectedData.length
    ? Math.round(
        selectedData.reduce((sum, t) => {
          const odds = oddsMap?.[t.combination];
          return sum + (Number.isFinite(odds) ? t.probability * odds : 0);
        }, 0) / selectedData.length * 10
      ) / 10
    : null;
...
// SKIP: 予測不安定
// ============================================================
function judgeV61(setMetrics, firstProbs, scenarios, stage) {
  ...
  const topScenarioKey = scenarios[0]?.key || null;
  // 1015Rバックテストの前半学習区間・後半検証区間の双方で再現した弱点。
  // 2まくり、4まくり、4まくり差しはBUYにせずWATCHでシャドー検証を続ける。
  const blockedProfitScenario = ["2_MAKURI", "4_MAKURI", "4_MAKURIZASHI"].includes(topScenarioKey);

  if (stage === "FINAL") {
    // BUY: 回収率110%+必須 + 1着信頼度 + セット確率 + シナリオ集中度
    if (recovery != null && recovery >= 110 && firstConfidence >= 50 && setProb >= 20 && topScenarioProb >= 25 && !blockedProfitScenario) {
      return { judgment: "BUY", reason: `V6.1利益型BUY(確率加重回収率${recovery}%・1着信頼度${round1(firstConfidence)}・セット確率${setProb}%・シナリオ${scenarios[0].label}${topScenarioProb}%)` };
    }
    if (blockedProfitScenario && firstConfidence >= 50 && setProb >= 20 && topScenarioProb >= 25) {
      return { judgment: "WATCH", reason: `V6.1弱点シナリオ除外(${topScenarioKey}) — シャドー検証継続` };
    }
    // WATCH: 予測強いが期待値不足
  ...
  // PRE: 回収率判定なし(オッズ未確定)
  if (firstConfidence >= 50 && setProb >= 20 && topScenarioProb >= 25 && !blockedProfitScenario) {
    return { judgment: "BUY", reason: `V6.1 PRE利益型BUY(1着信頼度${round1(firstConfidence)}・セット確率${setProb}%・シナリオ${scenarios[0].label}${topScenarioProb}%)` };
  }
  if (blockedProfitScenario && firstConfidence >= 50 && setProb >= 20 && topScenarioProb >= 25) {
    return { judgment: "WATCH", reason: `V6.1 PRE弱点シナリオ除外(${topScenarioKey})` };
  }
  if (firstConfidence >= 40 && setProb >= 15) {
...
// メイン: V6予想実行
// ============================================================
export function runPredictionV61(entries, race, settings, options = {}) {
  ...
  // 8. セット期待値
  const setMetrics = computeV61SetMetrics(selected, trifectas, oddsMap);

  // 9. 判定(EVベース)
  const { judgment, reason } = judgeV61(setMetrics, firstProbs, scenarios, stage);
