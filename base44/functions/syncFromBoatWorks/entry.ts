...
export default function SyncPanel() {
  ...
  const runApi = async () => {
    setBusy(true); setMsg(""); setErr(""); setDebugInfo(null);
    try {
      ...
      for (let i = 0; i < venues.length; i++) {
        ...
        for (let attempt = 0; attempt < 3 && !done; attempt++) {
          try {
            ...
            // 最初の1場だけ、実際に受信した生データを画面に残す(デバッグ用)。
            if (i === 0 && !debugInfo) {
              const d = res.data || res;
              setDebugInfo({ entry: d.debug_sample_entry || null, race: d.debug_sample_race || null });
            }
            done = true;
          } catch (e) {
      ...
      await load();
    } catch (e) { setErr(e?.response?.data?.error || e?.response?.data?.message || e.message || JSON.stringify(e)); }
    setBusy(false);
  };
    ...
    <div className="space-y-4">
      ...
      {debugInfo && (
        <div className="bg-white rounded-2xl border border-amber-200 p-4 space-y-2">
          <h3 className="font-bold text-sm text-amber-700">デバッグ: 受信した生データ(1件目)</h3>
          <div className="text-[10px] text-slate-500">entry</div>
          <pre className="text-[10px] bg-slate-50 p-2 rounded overflow-x-auto whitespace-pre-wrap break-all">{JSON.stringify(debugInfo.entry, null, 2)}</pre>
          <div className="text-[10px] text-slate-500">race</div>
          <pre className="text-[10px] bg-slate-50 p-2 rounded overflow-x-auto whitespace-pre-wrap break-all">{JSON.stringify(debugInfo.race, null, 2)}</pre>
        </div>
      )}
