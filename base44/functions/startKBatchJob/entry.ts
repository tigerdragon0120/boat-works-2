...
export default async function(req: Request) {
  try {
    ...
    const clean = items
      .map((x:any, i:number) => ({ file_name: str(x.file_name), payload_url: str(x.payload_url), order_index: i }))
      .filter((x:any) => x.file_name && x.payload_url);
    if (!clean.length) return Response.json({ error: 'valid items required' }, { status: 400 });

    const sr = base44.asServiceRole.entities;
    const batchId = `kbatch_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
    ...
    await sr.KBatchImportItem.bulkCreate(clean.map((x:any) => ({
      batch_id: batchId,
